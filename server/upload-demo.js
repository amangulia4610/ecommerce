...
const validateProduct = (product) => {
    ...
    // Discount validation
    if (product.discount !== undefined && (typeof product.discount !== 'number' || product.discount < 0 || product.discount > 100)) {
        errors.push('Discount must be a number between 0 and 100 if provided');
    }
    ...
    if (product.image && !Array.isArray(product.image)) {
        errors.push('Image must be an array if provided');
    }
    ...
    if (product.more_details && typeof product.more_details !== 'object') {
        errors.push('More details must be an object if provided');
    }
...
const createCategories = async () => {
    ...
    const uniqueCategories = [...new Set(
        sampleProducts.flatMap(product => product.categories || [])
    )];
    
    console.log(`📋 Found ${uniqueCategories.length} unique categories to process...`);
    ...
    for (const categoryName of uniqueCategories) {
        try {
            if (!categoryName || typeof categoryName !== 'string' || categoryName.trim() === '') {
                console.log(`⚠️  Skipping invalid category: ${categoryName}`);
                continue;
            }
            
            let category = await CategoryModel.findOne({ name: categoryName.trim() });
            ...
            console.log(`❌ Error with category "${categoryName}": ${error.message}`);
        }
    }
    
    console.log(`\n📊 Category Summary: ${createdCount} created, ${existingCount} existing\n`);
    return categoryMap;
};

// Upload products
const uploadProducts = async (categoryMap) => {
    let successCount = 0;
    let errorCount = 0;
    let skippedCount = 0;
    ...
    for (let i = 0; i < sampleProducts.length; i++) {
        ...
        try {
            console.log(`\n[${productIndex}/${sampleProducts.length}] Processing: ${product.name || 'Unknown Product'}`);
            
            // Validate product data
            const validationErrors = validateProduct(product);
            if (validationErrors.length > 0) {
                const errorMsg = `Validation failed: ${validationErrors.join(', ')}`;
                detailedErrors.push({ product: product.name || 'Unknown', errors: validationErrors });
            ...
            // Get category IDs with validation
            const categoryIds = [];
            ...
            const newProduct = new ProductModel({
                ...
                unit: (product.unit || 'piece').trim(),
                stock: parseInt(product.stock) || 0,
                price: parseFloat(product.price),
                discount: parseInt(product.discount) || 0,
                description: (product.description || '').trim(),
                more_details: product.more_details || {},
                publish: product.publish !== undefined ? Boolean(product.publish) : true
            });
            
            // Validate the model before saving
            const validationError = newProduct.validateSync();
            if (validationError) {
                const mongoErrors = Object.values(validationError.errors).map(err => err.message);
                throw new Error(`MongoDB validation failed: ${mongoErrors.join(', ')}`);
            }
            ...
            console.log(`❌ Error creating "${product.name || 'Unknown'}": ${error.message}`);
        }
    }
    
    return { successCount, errorCount, skippedCount, detailedErrors };
};

// Main function
const main = async () => {
    console.log('🚀 Starting comprehensive product upload demo...\n');
    ...
    try {
        ...
        const results = await uploadProducts(categoryMap);
        
        // Display comprehensive results
        console.log('\n' + '='.repeat(60));
        console.log('📊 UPLOAD SUMMARY');
        console.log('='.repeat(60));
        console.log(`✅ Successfully created: ${results.successCount} products`);
        console.log(`📌 Already existed (skipped): ${results.skippedCount} products`);
        console.log(`❌ Errors encountered: ${results.errorCount} products`);
        ...
        if (results.detailedErrors && results.detailedErrors.length > 0) {
            ...
            results.detailedErrors.forEach((error, index) => {
                console.log(`${index + 1}. Product: "${error.product}"`);
                if (error.index) console.log(`   Position: #${error.index}`);
                if (error.errors) {
        ...
        // Show success rate
        const totalProcessed = results.successCount + results.errorCount + results.skippedCount;
        const successRate = totalProcessed > 0 ? ((results.successCount / totalProcessed) * 100).toFixed(1) : 0;
        console.log(`📈 Success Rate: ${successRate}% (${results.successCount}/${totalProcessed})`);
        
        if (results.errorCount === 0) {
            console.log('\n🎉 All products processed successfully!');
        } else if (results.successCount > 0) {
            console.log('\n⚠️  Upload completed with some errors. Check the detailed report above.');
        } else {
            console.log('\n❌ Upload failed. No products were created successfully.');
        }
